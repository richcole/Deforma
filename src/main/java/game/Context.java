...
public class Context implements Runnable {
	...
	public void run() {
		...
		List<Geom> girlModelList = new ModelLoader().load("/home/richcole/models/Girl/girl.3ds", marble);
		CompiledMeshList girlModel = new CompiledMeshList(simpleProgram, girlModelList);
		...
		RenderableModel trees = new RenderableModel(new CompiledMeshList(simpleProgram, 
		...
		LineGeom line = new LineGeom(new Box(Vector.U1.times(-5), Vector.U2.times(5)), 0.2, marble);
		...
		CompiledMeshList xpsModel = xpsBuilder.toCompiledMesh(simpleProgram, marble, xpsBasicModel);
		CompiledMesh xpsBoneModel = xpsBuilder.getBoneCompiledMesh(simpleProgram, marble, xpsBasicModel);
		...
		for(int i=0;i<1;++i) {
			...
			xpsMesh.setModelTr(
				Matrix.translate(Vector.U3.times((i+1)*5))
					.times(Matrix.rot(Math.PI, Vector.U1))
					.times(Matrix.scale(5))
			);
