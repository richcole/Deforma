...
import game.image.ImageResource;
...
public class DSReader {
  ...
  private EventBus eventBus;
	...
	public DSReader(EventBus eventBus, File file) {
	...
	public class WordChunk extends Chunk {
		...
		Chunk parse(BReader reader, String indent) {
			...
			reader.readu8s((int)(length - offset));
	...
	public class FaceListChunk extends ContainersChunk {
		...
		Chunk parse(BReader reader, String indent) {
			...
			for(int i=0;i<n;++i) {
				for(int j=0;j<3;++j) {
					indexes[i*3+j] = reader.readu16();
				}
				info[i] = reader.readu16();
			}
	...
	public List<Geom> getMeshGeom(File baseDir, Chunk root, Material mat) {
		List<Geom> meshList = Lists.newArrayList();
		...
		for(Chunk parent: root.eachNode(0x4100)) {
			...
			for(Chunk chunk: parent.eachNode(0x4120)) {
				...
				for(Chunk fChunk: parent.eachNode(0x4130)) {
					...
					SingleLayerMeshGeom geom = new SingleLayerMeshGeom(material);
