...
import game.image.ByteBufferImage;
...
public class TgaLoader {
  ...
  public Image readImage(BinaryFileReader inp, long offset) {
    ...
    ByteBufferImage image = new ByteBufferImage(header.width, header.height);
    for(int j=0;j<header.height;++j) {
      for(int i=0;i<header.width;++i) {
        int x = (j*header.width+i)*(header.imageDepth / 8);
        image.write(header.pixels[x], header.pixels[x+1], header.pixels[x+2], (byte)0xff);
      }
    }
    inp.seek(mark);
    image.finish();
